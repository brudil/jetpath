  ...
  private scrollToBottom() {
    setTimeout(() => {
      requestAnimationFrame(() => {
        this.scrollElement ? this.scrollElement.scrollTop = this.scrollElement.scrollHeight : null;
      });
    }, 0);
  }
...
render() {
  ...
  return (
    <div>
      <button className={styles.button} onClick={props.panelOpen ? props.closeCommentPanel : props.openCommentPanel }><img src={chatIcon} width="32" /></button>
      <div className={cx(styles.root, { [styles.root_open]: props.panelOpen })}>
        <div className={styles.scroll} ref={ref => { this.scrollElement = ref; }}>
          {comments.map(comment => <Comment user={comment.user} created={comment.created} comment={comment.comment} key={comment.id} />)}
        </div>
        <div className={styles.postComment}>
          <PostComment onPost={(comment: string) => {
            props.postComment({ variables: {
...
const getVertical = () => connect((state: any) => ({
